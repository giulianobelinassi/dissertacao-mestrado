...
desempenho em máquinas \textit{manycores} no GCC: Paralelização via \textit{threads},
ou usando o mecanismo do \textit{Link Time Optimization} (LTO), para paralelizar a
aplicação das otimizações intraprocedurais do compilador, discutindo as
...
compilador. Conseguimos \textit{speedups} em um intervalo entre $0.7\times$ até $3.53\times$ ao compilar
arquivos individuais, e até $35\%$ de \textit{speedup} ao compilar projetos
inteiros usando um destes métodos, em contraste ao utilizar exclusivamente o
...
\abstract{
Compilers are enormous software designed to translate code in a language $A$ into
...
We managed speedups ranging from $0.7\times$ up to $3.53\times$ when compiling
individual files, and up to $35\%$ of speedup when compiling entire projects in
contrast to using exclusively \texttt{make -j} for parallel build. We also present the related works on the
topic, from parallel parsing to parallel optimizations, as well as a summary
about parsing and compiler theory.
}
