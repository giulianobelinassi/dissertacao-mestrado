...
valor 0, o processo que em sequência tentar decrementá-lo será bloqueado. A
sua execução só será retomada quando o semáforo for novamente incrementado
por outro processo \citep{semaphore}.

    Semáforos também podem ser utilizados para resolver o problema da
seção crítica, onde apenas uma \textit{thread} pode executar um determinado
trecho de código, embora seja mais comum a utilização de \textit{Mutexes}.
...
apenas uma deve estar executando a seção crítica.
Quando um \texttt{mutex} é travado, outra \textit{thread} que tentar
acessar a seção crítica será bloqueada pelo Sistema Operacional,
garantindo assim a seção crítica. A \textit{thread} só voltará a executar
assim que o \texttt{mutex} for desbloqueado.
\begin{figure}
      \begin{lstlisting}[
        language=pseudocode,
        style=pseudocode,
        style=wider,
        functions={},
        specialidentifiers={extern, call, async},
      ]
        extern mutex_lock, mutex_unlock
        mutex_t lock
...
é um ponto no programa onde, assim que a \textit{thread} executar, ela
será bloqueada até que todas as \textit{threads} cheguem ao mesmo ponto.
Isso é bastante útil para evitar com que \textit{threads} avancem na execução
do código enquanto ela deveria estar aguardando o resultado da computação
de outras \textit{threads}. Barrerias também costumam receber um parâmetro
de inicialização indicando quantas \textit{threads} precisam atingir a
barreira para que elas sejam desbloqueadas.

Por fim, exceto os semáforos, todos esses mecanismos mencionados acima são
implementados na biblioteca \texttt{Pthread}. Já o Semáforos, este está
disponível no Linux através da interface \texttt{semaphore.h}.
...
fila. Isso permite com que \textit{pipelines} sejam implementados, 
ou que um escalonamento dinâmico de trabalho possa ser implementado.
...
da fila, e ao inserir um dado nela, esse semáforo deve ser decrementado
...
\begin{figure}
      \begin{lstlisting}[
        ...
        style=wider,
        functions={},
        specialidentifiers={extern, call, async, int, class, mod},
      ]
        class Fifo
            int buffer[];
            int size;
