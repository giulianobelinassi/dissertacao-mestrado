...
\tituloport{Tópicos em Paralelização de Compiladores: Um Estudo de Caso no GCC}

% Se "\title" está em português, você pode definir o título em inglês aqui
\tituloeng{Topics in Compiler Parallelization: A Case Study in GCC}
...
\orientador[masc]{Prof. Dr. Alfredo Goldman vel Lejbman}
...
\begin{tabular}{rl}
    ...
    SSA          & \textit{Static Single Assignment}\\
    RTL          & \textit{Register Transfer Language}\\
    IPA          & \textit{Inter Process Analysis}\\
    API          & \textit{Application Programming Interface}\\
    WHOPR        & \textit{Whole Program Assumptions}\\
    LTO          & \textit{Link Time Optimization}\\
    LLVM         & \textit{Low Level Virtual Machine}\\
    GNU          & \textit{GNU is Not Unix}\\
    GCC          & \textit{GNU Compiler Collection}\\
    OS           & Sistema Operacional\\
    LGEN         & \textit{Local Generation}\\
    WPA          & \textit{Whole Program Analysis}\\
    LTRANS       & \textit{Local Transformations}\\
    LTO          & \textit{Link Time Optimization}\\
