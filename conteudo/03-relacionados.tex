...
concurrent parsing techniques for the \LR{$k$} family. He proposed a parallel
\LR{$k$} by breaking the input into several arbitrary parts and running a serial
...
of the \LR{$0$} algorithm. Their idea consisted of breaking the input arbitrarily
...
These presented approaches were developed not to improve compilation time or
improve parallelism usage, but to analyze the program as a whole, resulting in
better optimizations. In our work, we will present a way to use the LTO engine
to compile single files in parallel, therefore we want to improve compilation
performance, not improve the quality of the optimizations, which would be as
good as the classical compilation scheme. 
