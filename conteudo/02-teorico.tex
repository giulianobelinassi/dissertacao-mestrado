...
First, we present all parts of the compiler pipeline to understand the
compilation phases. Following that, we discuss how the GCC compiler presents
these topics implemented, and how these parts interact with each other.
Finally, we present some basic concepts and algorithms about Parallel
Computing.
...
\begin{section}{Finite State Automatas}

A large part of compiler theory relies upon parsing text, and the simplest
machine we can think about this is a Finite Automata. With these machines, we
can easily build several controller machines for various applications, such as
elevator state controllers, microwave control panels, signal detectors, and so
on. This is because automata can be cheaply and easily be implemented using
flip-flops, an electronic device which can hold a bit as data storage.
...
can easily convert them into a finite automata.
...
A \texttt{flex} regex for it would be
$$\land\texttt{[0-9]+\$}$$
...
the $\delta$ function will be defined as:
	\[
		...
		\end{cases} \nonumber
	\]
Figure \ref{fig:a_automata} illustrates this machine.

Deterministic automatas can be easily implemented using a transition
...
\ttt{DFA}, which we will not present in this work.


\begin{definition}
A Nondeterministic Finite Automation (\ttt{NFA}) is a 5-tuple
$(Q. \Sigma, \delta, q_0, F)$, where
\begin{itemize}
...
\ttt{NFA}s are interesting machines, once they are often used to implement
Regular Expressions via the Thompson Construction \citep{dragonbook}. However,
implementation should be done with care, as these machines requires \textit{backtracking},
which may lead to some unfortunate bugs \citep{PR86164}.

Automatas, however, are not very expressive machines. For example, consider
the following language:

$$L_2 = \conj{\ttt{(}^n \ttt{)}^n}{n \geq 0}$$

This is a stripped down version of the balanced parenthesis
problem. Unfortunatelly, it is impossible to build an finite automata
that can decide if a word $w \in \Sigma^*$ is in $L_2$.

\begin{lemma}
There is no finite automata recognizing $L$.
\end{lemma}
\begin{proof}
Let $\mathcal{A} = (Q, \sigma, \delta, s, F)$ be such automation,
with $\Si = \{\ttt{(}, \ttt{)}\}$.
Since it is a finite automation, then $|Q|$ is finite, and therefore
$|Q| = n$. Now take the following word $w = \texttt{(}^{2n}\texttt{)}^{2n}$.
...
circuit. Note that $x$ and $y$ constists of a sequence of $\texttt{(}$
characters, because we are processing the first $2n$ characters of $w$.
Since $y$ is the part of $w$ in this circuit, then
$ xyyz $
is also recognized by $\mathcal{A}$. Let $k = |y|$. This means that
the input string
$$ \texttt{(}^{n+k}\texttt{)}^n \not\in L_2$$
is also recognized by $\mathcal{A}$, which means that such machine does
not exist, and therefore there is no finite automata which can detect
$L_2$.
\end{proof}

In fact, this proof is heavily based on the proof of the Pumping Lemma,
a powerful tool to find if a certain language is not regonizable by an
Finite Automata. Entering in details about this is beyound the scope of
this work.

\end{section}

\begin{section}{Pushdown Finite Automatas}
	An extension of the Finite Automatas are the Pushdown Finite Automatas,
which is simply an Nondeterministic Finite Automata together with a infinite
stack. This machine as it is presented is not very useful for building
compilers, however, they are important for defining a Deterministic Pushdown
Automatas, which are the key for defining Deterministic Context-Free Grammars,
which can be parsed with $O(n)$ algorithms.
...
the empty string, \textit{i.e}., if $w \in \Sis$, then
$\lambda w = w \lambda = w$.

Those machines starts with an empty stack, and it will then choose
to read an input based on the following notation given a transiction from $q_i \in Q$
to $q_j \in Q$:
...
a final state $q_f \in F$ consuming the entire input string $w$.

These machines are more powerful than Deterministic and Nondeterministic Finite
Automatas. For instance, there is a Nondeterministic Finite Automation detecting
$L_2$, in which we proved that there is no Deterministic (and therefore there is
also a nondeterminisic) Finite Automata for such language.
...
Figure \ref{fig:pushdown_automata} describes a Nondeterministic Finite Pushdown
Automata which detects $L_2$. This machine works as follows: On the transition
...
at $q_2$, and pushing then on the stack, or avance without reading, poping or
...
characters on the stack.  If the machine incorrectly guesses when to switch
...
would not be completly consumed, and therefore the machine will reject the
string. The only way that an string is accepted is when $j = i$. Notice that
this is not a formal proof that such machine works.

However, as stated in the begining of this section, Nondeterministic Finite Pushdown
Automata are not a very important machine when the topic are compilers. On Figure
\ref{fig:pushdown_automata}, a careful reader may notice that backtracking is
required to implement such machines because on given state, there can be
multiple transictions on the same character. For instance, on above machine,
when on state $q_2$, given an input character $\ttt{(}$, the transiction to
$q_3$ is valid, since $\{(q_3, \lambda)\} \in \delta(q_2, \lambda, \lambda)$,
as well as $\{(q_2, \ttt{(})\} \in \delta(q_2, \ttt{(}, \lambda)$.
We are interested in a less powerful machine, the Deterministic Finite Pushdown
Automata, which does not require Backtracking.

\begin{section}{Deterministic Finite Pushdown Automata}
	Let's now restrict the Nondeterministic Finite Pushdown Automatas so that
it is possible to remove the necessary backtracking from these machines.
The way to do it is to restrict when the machine can have certain transictions
with regard to $\delta(q, \sigma, \lambda)$, $\delta(q, \lambda, \gamma)$ and
$\delta(q, \lambda, \lambda)$.

\begin{definition}
A Deterministic Finite Automation can be defined as a 6-tuple
$(Q, \Si, \Ga, \delta, q_0, F)$, where
\begin{itemize}
...
$\delta$ must also satisfy the following condition: For every $q \in Q$, $\sigma \in \Si$,
...
distinct actions given the same input, as it is possible in the Nondeterministic
version by setting both $\delta(q, \sigma, \gamma) \neq \emptyset$ and
...
happens the same way as the Nondeterministic version.

Let's now build a Deterministic Finite Pushdown Automata for $L_2$. We can start
from the Nondeterminisitc version, since the only condition that is violated
there with regard to a Deterministic machine are the transitions
$\delta(q_2, \ttt{(}, \lambda)$ and $\delta(q_2, \lambda, \lambda)$: only one
of these transictions can exist. We therefore can design the following machine
to address these problems, eliminating the $\delta(q_2, \lambda, \lambda)$
transiction and adding an extra final state so that it accepts $\lambda \in L_2$.
...
can not be detected by such machines.
...
\begin{lemma}\label{lemma:pushdown_2}
	For every Deterministic Finite Pushdown Automata $\mathbb{P}$, there is
	an equivalent Nondeterministic Finite Pushdown Automata that detects the
	same language.
\end{lemma}
\begin{proof}
	See \ref{sipser:2012}
...
Lemma \ref{lemma:pushdown_1} and \ref{lemma:pushdown_2} shows that the Deterministic
Finite Pushdown Automatas are not so powerful when compared to the Nondeterminisitc
version.
...
\begin{section}{Context-Free Languages}

A more powerful way to express languages are through Context-Free Grammars.
\begin{definition}
...
$V = \{S\}$, $\Si = \{\ttt{(}, \ttt{)}\}$, $R = \{S \rightarrow \ttt{(}S{)}\}$
can recognize $L = \conj{\ttt{(}^n \ttt{)}^n}{n \geq 0}$.

Context-Free Grammars are very powerful tool to express how a Programming Language
...
Usually, Lexical Analysers are implemented using Automatas for several reasons,
such as an attractive computational complexity of $O(n)$, where $n$ is the
input size, the existence of algorithms to convert a regular expression to an
Automata \citep{thompson}, and by the existence of algorithms to minimize the
size of the Automata \citep{hopcroft1971n}. This allows that automatic Lexical
Analysers generators such as
\textit{Flex}\footnote{https://www.gnu.org/software/flex/} be extremely
efficient.
...
provided by the Lexical Analyser. In this process, it certifies that the Input
Code respects the language's grammar, pointing errors otherwise.
...
Syntactic Analysers relies on Nonambiguos Grammars that generates a
deterministic Context-Free Language, once general grammars require more
computationally expensive algorithms \citep{sipser2012}.  There are several
algorithms to realize this analysis, but we must highlight:
...
%\citep{sipser2012}.
%Existem vários algoritmos para realizar a análise sintática,
%mas destacam-se:
\begin{itemize}
...
\item Recursive Descent: A predictive algorithm, which tries to match the
grammar from the root to the leaves. Here, every non-terminal token is
represented as a function, which we can simply call when reducing a rule. This
results in a easy to implement algorithm, however, it has some limitations.

For instance, assume that a grammar G have the following rule:
$$A \rightarrow Aw $$

A code to parse this rule would be implemented as:
...
Which would result in an infinite loop. In fact, when implementing a Recursive
Descent algorithm to parse a certain grammar G, it must be free of
left-recursion.

    \item LL($k$) -- \textit{Left to Right, Leftmost Derivation, k lookahead}: Outro algoritmo de caráter preditivo, que tenta construir a AST da
    ...
    \item LR($k$) -- \textit{Left to Right, Rightmost Derivation, k lookahead}: Um algoritmo que tenta construir a AST das folhas para a raiz.
