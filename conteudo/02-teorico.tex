...
we denote the following definitions in this work: denote \textit{Source
...
compiler; and \textit{Target Language} as the language in which the compiler must
generate the code.
...
languages such as C, into assembly language, and those are the focus of our work.
...
that are heavily used in a compiler. In Section \ref{sec:optimization}, we
...
computing, which we use in this dissertation. Finally, Section
...
Computer programs are written as a text file in some programming languages,
...
More details about how to construct parsers, what a grammar is in the context
...
it not only does a direct translation into another language, but it
tries to generate faster code than the input code created by the programmer
while still maintaining its correctness.

There are several data structures and frameworks which are often used to
...
evident that can be simplified further to $y = 2 \times t_1$, generating the
...
belong to distinct basic-blocks. Figure \ref{fig:1-norm_program:cfg} shows
...
if a certain node is reachable from the CFGraph source node. However, its main usage
...
\begin{definition}
	A poset $\mathcal{P} = (L, \sqsq)$ is a \textbf{meet-semilattice} if
	for every non-empty subset $S \ssq L$, $\left(\bigsqcap_{s \in S} s \right) \in L$.
\end{definition}

There are also structures that satisfy stronger claims, which is the case
...
lattice. A meet-semilattice is enough.
...
lattice, we are interested in monotone functions, which help to ensure that
some analysis end after a finite number of iterations.
...
what properties holds to decide for an optimization.  Incorrectly guessing
...
Given a Control-Flow Graph $G = (V, E, s_0, F)$ of a program, it constructs a
...
$\mathcal{F}_G$ is related to each node of the $G$. Furthermore, Figure
...
turn to be live at vertex $v$, and $\texttt{kill[} v \texttt{]}$ are the
variables in which liveness is no longer valid at vertex $v$. For instance,
...
\ref{eq:liveness} in Table \ref{fig:table_cfg} using the
...
\end{tabular}
\caption{Solution of the Equation \ref{eq:liveness} for Figure \ref{fig:cfg_liveness} 
...
\begin{subsection}{Dominator Tree}
	One data structure used on optimization is
	...
	\cite{georgiadis2006finding} present a forward dataflow equation
...
\end{figure}
	...
	faster algorithm (but more complex than the dataflow equation) to
...
and last statements can be discarded as well if there is no further use of $x$
and $y$ in the program. However, how can the compiler know that? The SSA form
makes this clear. The idea is that variables can not be redefined and only new
variables can be created, which motivates the following definition.
...
on the right-hand side of any assignment (it has no use in the \textit{def-use}
chain), meaning that it can be discarded; which will then imply that $y_1$ will
be unused and can be discarded as well.

However, the SSA form can get trickier to implement if there are jumps in the
...
that will select the correct variable. Figure \ref{fig:code_ssa_form}
illustrates this.
...
If a variable previously had two definitions and the second has no use, SSA
...
function call, the call itself consumes time; therefore, it may be
interesting to copy and paste the content of the function being called and
remove it completely.  Therefore, a data structure is required to model the
...
\textit{summary} of the function instead of the entire function; that occurs because the
program being compiled often is large enough so there is not sufficient
...
Registers are very fast, but small memories inside a processor, in which data is
required to be loaded for operations like addition, subtraction, multiplication, etc. These
memories are a very scarce resource in a processor; therefore, it
...
infinite number of variables, the goal here is to rewriting the code such that it
...
This information is used to build an \textit{interference graph}, which is presented as an
example in Figure \ref{fig:interf_graph}.
...
registers are necessary and where to allocate them once the graph is colored.
...
if $G' = G - \{v\}$ can be colored, then so does $G$, because one can simply select
...
than a way to translate a statement into machine code, for instance, 
...
cycles to execute. This is especially important for CISC-based processors,
where a sequence of instructions can be replaced with a single instruction,
...
of being a fast method to implement a way to translate the code, but
it very often generates poor final code, as illustrated in
Figure \ref{fig:macro_exp}. Some optimizations can be employed
to improve the result of such translations, such as
\textit{peephole optimization}, that searches for unnecessary memory operations
that annulate themselves in a small window.
...
\textit{reduce-reduce} conflicts.
	...
	\begin{itemize}
		\item On \textit{shift-reduce} conflicts, always choose to shift. The
		intuition is that larger patterns will result in more optimal code
		since it allows more information to be gathered by the parser. However,
		the parser could simply select to \textit{shift} when it should have
		\textit{reduce}d. Therefore, either backtracking is necessary here when
		the parser takes the wrong decision, or the grammar designer provides
		additional rules to avoid such cases.

\item On \textit{reduce-reduce} conflicts, it selects the rule with the
longest pattern. The reason is also that the longest patterns will generate
better codes. However, even this could also result in a parsing error. The
solution is to devise a rule that can be used as a fallback, and the
...
implement a new \textit{backend} to the compiler. However, the main issue, however, is
the high number of states required by the
parser \citep{blindell2016instruction}, which often makes it impractical.
...
$P_n$ with the set of patterns possible to be applied to $n$. Therefore, the
...
Compilers are large programs, so there is a great interest in
...
\item A Front-End for each Source Language. In this module, it is executed both
the lexical and the syntactical analysis to construct the Abstract Syntax Tree
(AST) and populating the Symbol Table. Following that, this structure is
converted to another compiler's IR.
...
optimization is such that there is no dependency regarding the Target
Language (often, assembler of some processor). Example: \textit{constant propagation}.

\item A Back-End for each Target Language, responsible for translating the IR into
...
optimization is such that there \textit{is} a dependency regarding to the
...
written in the Source Language. From a Software Engineering perspective, it
means that in order to add support to a new Source Language, one could simply
write a new Front-End. It is also possible that some classes are shared between
...
Here, we construct data structures that might be necessary for the
...
into other IR, which may be more closely related to the target hardware.
After that, the control is transferred to one of the compiler's Back End, where it
will be transformed to the final language.

\end{subsection}

\begin{subsection}{Back-End}

The Back-End is responsible for the final translation into the target
...
another without any modification to the Middle-End. Thus, the code can be
...
Compilers can optimize for several metrics, not exclusively for execution time.
...
are you optimizing to. This is where a simple but powerful modularization
appears: the \textit{compiler passes}.
...
on). It also has a \textit{gate} method, which will determine if this optimization
...
Richard Stallman with its first public release in March of 1987. Back then,
...
optimizations are done, and also to make the development of a Front-End more easily.
...
pattern and then replaces it for another. Such patterns are expressed in the file
...
%\citep{sinatan}.
	...
	Finally, each Front -End of GCC is responsible into translating the source
	code into this intermediate language. To visualize a program in this representation,
	...
	directly into GIMPLE \citep{gimple}.
...
to a 3AC representation, and having a further step into lowering this
...
return statements and converting try-catch control flows to exception regions.
...
\begin{subsection}{Optimization Passes}\label{sec:opt_passes}
    ...
    \begin{enumerate}
		...
		callgraph of the program being compiled. These have often three step passes:
		on the first run, it collects a \textit{summary} of the function, then
		...
		is that the \textit{analyse} phase must be able to run without having the
		entire function loaded in memory.

        \item \textbf{GIMPLE} - A Hardware-Independent Intraprocedural pass.
		It can do some analysis in the code (such as collecting some metric)
	...
	requires only one step. A sketch of the passes class is illustrated in Figure \ref{fig:opt_uml}.
...
\begin{itemize}
    ...
    \item The driver \texttt{gcc}, responsible for calling the compilers \texttt{cc1},
		...
		responsible for interacting with other tools from the toolchain;
...
collect all generated object files and link them together
...
This process is traditionally used to compile complex software along with
...
then encapsulated into an object file), then every file is linked along with
\textit{ld}. In GNU Make, it is possible to explicitly determine the
dependencies of each file, and the compilation can be done in parallel between
...
includes) Therefore, every optimization is applied only locally to its TU.
...
contributors worldwide, whether it is paid or not. As with every Free Software project,
...
GCC is written in C++11; therefore you need a working C++11 compiler to
...
\begin{itemize}
	...
	\item \texttt{-{}-disable-bootstrap}: it disables a feature called
	...
	spent to compile the project.

	\item \texttt{-{}-enable-checking=release}: it disables most sanity checkings
	in the compiler. This is useful for testing the compiler performance or to
	build a compiler that may be used in production.

	\item \texttt{-{}-disable-multilib}: it disables support for 32-bit x86 binaries on
	amd64 machines. This requires additional packages in your system that you may
	not want to install.

	\item \texttt{-{}-enable-languages=c,c++}: it compiles only C and C++ compilers and
...
where \texttt{<num\_jobs>} are the numbers of cores available in your computer.

If there is a missing library in your system, the building process will notice
...
In order to install GCC in a custom folder (for instance, in \texttt{/tmp}), you
should install it by using an additional parameter \texttt{DESTDIR}. For instance,
...
If it shows the current version you compiled, then everything is
...
development, and take down notes about the tests that are failing.

You can run the extensive GCC testsuite by launching \texttt{make} as:
...
There are two cases that require a distinct approach. One is if you want to
...
In the first case, you should launch the compiler with:
	...
	for calling the correct compiler, and this wrapper will simply make things
	easier for the user by instructing the driver to call the compiler under gdb.
...
\cite{stallman1988debugging} provide a walkthrough on how to use it.

\end{subsection}

\begin{subsection}{Writing and Sending Patches}

Writing patches is quite straightforward. First, be aware that GCC uses the GNU
Coding Style; and therefore your text editor must be configured accordingly. In
the \texttt{contrib} folder, there are some configuration files for Vim, which
you should be using if you code using such text editor. Perhaps one of the most
bureaucratic steps in Free Software development is sending the changes back to
the community for review. However, once you master this process, sending
changes comes naturally and can be done quite fast.
...
\end{align}
Then, the text editor window will show up for you to explain what this
...
of how to create a detailed changelog, read the file
...
\end{align}
	...
	patch. Now you should place your commit (which is in \textit{feature})
	...
	everything is correct, then you can send the patch by using:
\begin{align}
...
\begin{subsection}{Where to Begin}
	...
	a good start is:
	\begin{itemize}
		\item Checking if you can come with a new \texttt{match.pd} rule. This
		...
		\item Reading the EasyHacks page\footnote{https://gcc.gnu.org/wiki/EasyHacks}.
	\end{itemize}
...
is slowing down, while the number of cores has been growing exponentially
...
\begin{subsection}{Parallel Programming in Shared Memory}
	...
	through a bus to all processors, allowing the reading and writing of data.
...
%travas e sincronização.
	...
	therefore, a brief explanation is required about its abstractions.
...
for its stack and the Thread Local Storage (TLS) region, which makes
...
issue revolves around finding \textit{what} must be shared and how to do an
...
other thread reaches the same point, as illustrated in Figure \ref{fig:barrier}.
...
reach the barrier so they can be released to continue its execution.
...
Apart from semaphores, every mechanism mentioned above is implemented in the
...
Therefore, to compute energy measuring the power consumption of a system, we use: $$P =
...
the sensor in discrete time, and not continuously.  Thus, we can
...
itself by representing the real value in between a range of two values, and
