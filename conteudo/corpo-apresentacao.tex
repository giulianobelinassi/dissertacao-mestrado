    ...
    \begin{itemize}
        \item Estudo do gargalo na compilação do GCC usando o \texttt{gimple-match.c}
            \begin{itemize}
                ...
                \item 75\% ($57s$) é gasto em otimizações Intra Prodecurais e Geração de código
...
\begin{tikzpicture}
    ...
    \draw[->]    (gcc_lto.east)   to [out=0, in=180] (gcc_wpa.west);
...
\begin{frame}{Teorema do Limite Central}
    \begin{itemize}
        ...
        \item Com isso, podemos definir um intervalo de confiança para a média amostral $\overline{X}$ 
    \end{itemize}
\end{frame}
...
\begin{frame}{Cálculo de Energia Consumida}
    \begin{itemize}
        \item Discretizando $dt$ em $\Delta t = t_{i+1} - t_{i}$ com $n$ pontos, define:
            ...
            $$\sum_{i=0}^{n-1} p_i(t)\Delta t \leq E \leq \sum_{i=0}^{n-1} P_i(t)\Delta t$$
