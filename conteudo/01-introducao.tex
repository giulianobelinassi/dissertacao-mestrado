...
The recent advances in both technological and computational fields induced a
huge expansion of the software system ecosystem. New programs are created to
supply the needs of the most diverse domains, either through web systems coded
in script languages; or by components to an operating system destined to
control some hardware resources. Independently of the reason behind its
development, it is true that such code will be transformed into machine
language by a compiler or assembler, even if it is executed by an interpreter. 
...
develop and improve them, such as the GNU Compiler Collectioni
...
language is often called a Transpiler.
...
Although it is possible to write code in machine language, turning a compiler
...
The GCC is a largely used  Open Source compiler both in the academic and
industry environments. Being a large project, GCC has an additional language
created to better express and facilitate optimizations in code generation, that
is compiled into C++ to (1) reuse the optimizations already implemented in the
compiler, and (2) avoid rewriting several functionalities already implemented.
Unfortunately, this process highlights a bottleneck in the project compilation
on manycore machines, because GCC is not capable of compiling a file in
parallel in an incremental way.

Currently, all parallelism in the compilation stage is provided eighter by the
...
\citep{mailgcc}.  Another solution would be split the files into smaller files,
and use the parallelism feature already present in GNU Make, but this implies
in a modification to the project structure, which may not be ideal.
...
Since today the processors' manufacturers are increasing its products power
through parallelism over raw sequential power, compilers that can profit from
this resource could reduce the time taken when compiling a project, or even
reduce execution time on script languages that rely on JIT compilation. It
means a reduction in resource usage, such cases such as power, time, and thus
money. So far, every parallelism found internally in optimizing compilers was a
consequence of a structure created to allow more aggressive optimizations, such
as the LTO \citep{glek2010optimizing}.
...
\begin{description}
    \item[\textit{RQ1}] - In which points a compiler can use from parallelism?
...
Compiler Theory and Parallel Computing, to clarify some topics about this work.
